...
Comenzamos implementando el esquema \eqref{esquemacompletoonda} y así resolviendo la ecuación de onda en ese contexto.
 \begin{verbatim}
 	...
 	#derivada lo haremos con una aproximación progresiva del tiempo
 	def g(x):
 	return 0
 	...
 	def f(x):
 	return np.sin(np.pi * x)  # Tener cuidado con la función
 	 por si se sale del rango
 	
 	
 	
 	# Definimos una función para usar en el método explícito y
 	# poder cambiarlo de manera rápida si hace falta
 	# Si queremos hacerlo en un file independiente tendremos 
 	# que añadir el mu
 	...
 	start = time.time()
 	print("Vamos a hacer una resolución numérica de la 
 	ecuación de onda u_tt-c^2u_xx=0")
 	print("Introduce c:")
 	c= float(input())
 	print("Introduce el nombre del directorio donde quieres 
 	...
 	l=1
 	tmax= 1
 	m = int(l*3)# Constante representativa del número de trozos
 	# en los que separamos el espacio
 	h=l/m  #Cambiamos la h para que sea entera
 	
 	k =h/c #Lo tomamos de esta forma para ganar en exactitud 
 	#pagina 133 numerical sound synthesis
 	#k =h/c * 0.01
 	#k =h/c * 0.1
 	#k =h/c * 0.5
 	#k =h/c * 0.9
 	...
 	# Creamos un mallado de puntos en los que aproximaremos 
 	#la solución
 	...
 	u[1:m, j] = Explicito(u[2:m+1,j-1], u[0:m-1,j-1], u[1:m,j-1],
 	u[1:m,j-2],mu)
 	...
 	np.save(directoriofinal+'constantes', [c, m,h,n,k,mu,tmax] ) 
 	# Guardar en cons las constantes que necesitemos para usarlas 
 	luego en el guardado
 \end{verbatim}
...
\begin{verbatim}
	...
	plt.ylabel('u')
	#axis tight
	plt.title(f'Movimiento del punto {x[j]} en el espacio')  
