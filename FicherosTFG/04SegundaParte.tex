...
\end{figure}
\vspace{1cm}
Al encontrarnos en una onda estacionaria, $\lambda$  dependerá del largo de la cuerda, $L$, y del número de nodos que se presenten en la misma, $N$, de la siguiente forma $\lambda=\fra{2l}{N-1}$.\\

\begin{wrapfigure}[6]{r}{0.45\textwidth}
	\vspace{-1.5cm}
	\includegraphics[width=0.45\textwidth]{2Seno4.png}
	\caption{$sin(2\pi x)$ en $x=0.25$ Archivo figura5-37.wav}
	\label{fig:2Seno4}
	
\end{wrapfigure}
\vspace{2cm}
Luego, si queremos obtener $\textbf{f}=440Hz$, frecuencia del La central, será $\lambda=\frac{2}{2}=1$,  $c=\lambda \textbf{f}=1*440 = 440$. Con estos parámetros, para el punto de máxima amplitud $x=0.25$\\
...
Podemos observar esto  eligiendo los puntos adecuados, $x=0.5$ y $x=0.25$, para seno de $\pi x$, \ref{subsec:sin(x)},  y seno de $2\pi x$. Vemos claramente que llega a su máxima amplitud el la mitad de veces.
