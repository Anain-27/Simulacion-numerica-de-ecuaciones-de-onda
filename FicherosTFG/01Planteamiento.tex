...
\begin{definicion}[label={Escalar},nameref={Title or anything else}]{Producto escalar o interno.}
	Sea $E$ un espacio vectorial. Una función $(\cdot,\cdot):E\times E\rightarrow F $ se llamará producto escalar o interno si para todo $u,v,w\in E$ y los escalares $\alpha, \beta \in F$ se satisfacen las siguientes propiedades:
	\begin{itemize}
		\item Linealidad: $ (\alpha u+\beta v,w)= a(u,w)+b(v,w)$ .
		\item Simetría: $ (u,v)=(v,u)$ .
		\item Positividad: $ (u,u)\leq 0$ , y $ (u,u)=0$  si y solo si $x=0$ .
...
En nuestro caso trabajaremos sobre $L^2(E)$, que es un espacio de Hilbert con el siguiente producto interior:



\begin{definicion}[label={Inner},nameref={Title or anything else}]{Producto interior en $L^2$}
	Sean $f$ y $g$ dos funciones definidas en $E$, definimos el producto interior sobre $L^2(E)$, de ellas como:
...
\begin{figure}[!ht]
	...
	\caption{Mallado}
\end{figure}
Una vez ya definido este mallado retomemos la definición del producto escalar para los puntos del mismo.\\

Siguiendo la definición de producto escalar de \cite{TimeDependent}. Puesto que queremos utilizar el producto escalar sobre una partición del interior del intervalo espacial $(a,b)$, lo que utilizando nuestro mallado queda $D=\{ x_1,\dots,x_{m-1}\}$, definiremos el producto interior sobre $\mathbb{R}^{m-1}$.
