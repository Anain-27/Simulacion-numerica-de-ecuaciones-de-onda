...
\begin{definicion}[label={Escalar},nameref={Title or anything else}]{Producto escalar o interno.}
	Sea $E$ un espacio vectorial. Una función $(\cdot,\cdot):E\times E\rightarrow F $ se llamará producto escalar o interno, si para todo $u,v,w\in E$ y  $\alpha, \beta \in F$ escalares, se satisfacen las siguientes propiedades:
	\begin{itemize}
...
Siguiendo la definición de producto escalar de \cite{TimeDependent}. Puesto que queremos utilizar el producto escalar sobre una partición del interior del intervalo espacial $(a,b)$, el cual utilizando nuestro mallado queda $D=\{ x_1,\dots,x_{m-1}\}$, definiremos el producto interior sobre $\mathbb{R}^{m-1}$.
