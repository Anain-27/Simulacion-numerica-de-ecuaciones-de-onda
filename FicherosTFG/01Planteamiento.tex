...
Una vez ya definido este mallado retomemos la generalización de \ref{innermallado}. 

\begin{definicion}[label={ProductoDiscreto},nameref={Title or anything else}]{Producto interior en dominios discretos}
	Sean $u$ y $w$ dos funciones definidas en nuestro mallado y tomemos $D$ como la partición del interior del intervalo espacial, es decir, $D=\{ x_1,\dots,x_{m-1}\}$. Para ello debemos fijar un instante temporal $t_r$. En este contexto, definiremos el producto interior sobre $D$ como:
	\begin{equation}\label{eq:interiordiscretomalla}
		\left(u(\cdot,t_r),w(\cdot,t_r)\right)_{D}= \sum_{j=1}^{m}h u(x_j,t_r)w(x_j,t_r),
	\end{equation}
	Para este tipo de casos usaremos la siguiente notación:
	
	\begin{equation}\label{eq:notacioninteriordiscretomalla}
		\left(u_{\cdot,r},w_{\cdot,r}\right)_{D}= \sum_{j=1}^{m}h u_{j,r}w_{j,r}.
	\end{equation}
	Observamos que estos últimos términos se salen del mallado definido, estos serán puntos virtuales del retículo.\\
...
\begin{definicion}[label={derivada2},nameref={Title or anything else}]{Aproximación centrada de la derivada de segundo orden}
	...
	\begin{equation*}
		\begin{split}
			\delta_{x}u_{j,r}&=  \delta_{x^+}\delta_{x^-}(u_{j,r})=\frac{1}{h^2}\left(e_{x^+}u_{j,r}-2u_{j,r}+e_{x^-}u_{j,r}\right)=\\
...
\begin{lema}[label={SumaPartes},nameref={Title or anything else}]{Representación de la suma por partes}
	...
	\begin{equation}\label{eq:rep2}
		\left(u_{\cdot,r},\delta_{x^-}w_{\cdot,r}\right)_{D}= -\left(\delta_{x^+}u_{\cdot,r},w_{\cdot,r}\right)_{D}-u_{0,r}w_{-1,r}+u_{m+1,r}w_{m,r}.
	\end{equation}
...
Estas igualdades son fácilmente demostrables.
\begin{proof}
