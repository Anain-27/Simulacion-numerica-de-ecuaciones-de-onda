...
Podemos encontrar diversas notaciones y formas de hacer este análisis en libros clásicos y modernos, véase \cite{Software, smith1988analisis, Strikwerda}.\\
...
Tal y como hacíamos en \ref{energia}, querremos verificar si el principio de la conservación de la energía se cumple en nuestro esquema numérico. Al ser el esquema discreto, tendremos que usar la definición de producto interior discreto \ref{ProductoDiscreto},
en lugar de la del producto interior anteriormente utilizada en la sección \ref{sec:energiaondas}.\\

Comenzaremos tomando como espacio a tratar el mallado \ref{Mallado}. Puesto vamos a utilizar el concepto de suma por partes, parece conveniente definir $D$ como la partición del interior del eje espacial, ya que, es en este donde tenemos definido nuestro esquema \eqref{esquemacompletoonda}.\\

Sin embargo, en nuestro caso nos beneficiaremos enormemente de utilizar $\hspace{2cm} D=\{x_0,x_1,\dots, x_{m-1}\}$. Por tanto, vamos a analizar de que forma podría definirse nuestro esquema para los puntos de la forma $(x_0,t_r)$, con $r=1,\dots, n-1$.\\

Evaluemos la ecuación \ref{ecuaciononda}, en el punto $(x_0,t_r)$
...
Vemos que al igual que  \eqref{eq:nodovirtual}, obtenemos un nodo virtual, el cual, si definimos de la siguiente forma
...
Retomando el tema que nos atañe, la energía en nuestro esquema, ya con  $D=\{x_0,x_1,\dots, x_{m-1}\}$, y utilizando en este caso la función $\delta$ definida en \ref{defdelta}, para hacer el producto interior discreto con $\delta_tu$, tendremos.
 

$$\left(\delta_{tt}u,\delta_tu\right)_D= c^2 \left(\delta_{xx}u,\delta_tu\right)_D. $$

Siguiendo la notación que tomábamos en \ref{SumaPartes}, para cada $r=1,\dots,n-1$ tendremos:
...
$$\delta_{t}E(u)=\mathfrak{b}.$$

Por lo tanto, para que se conserve la energía tendrá que ser $\mathfrak{b}=0$. Veamos que es así.\\

Utilizando la condición de contorno para \eqref{eq:esquemafrontera3}, para $r=1,\dots, n-1$,
