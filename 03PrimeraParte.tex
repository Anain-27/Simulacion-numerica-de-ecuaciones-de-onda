...
y $x_0=0$, $x_m=L$, es decir es una partición de nuestro dominio espacial $(0,L)$.\\
...
Con las particiones ya definidas, haremos un mallado del tipo $(x_j,t_r)$.\\
...
A continuación nos centraremos en dichas condiciones, siguiendo \citep{strauss}. En primer lugar la condición inicial \eqref{eq:frontera1} nos dará los valores de los nodos 
 ...
 \end{equation*}
  ...
  Por otra parte, las condiciones de contorno de tipo Dirichlet \eqref{eq:frontera3} quedarán como $u_{0,r}=u_{m,r}=0 $ para $r=0,1,\dots,n$.\\
  
  Para la condición faltante, \eqref{eq:frontera2}, podrían elegirse diversas opciones, podemos encontrar abundantes textos al respecto, como por ejemplo \citep{leveque}.  En nuestro caso no ahondaremos mas en este tema y emplearemos la expresión más frecuente. Haciendo una aproximación por cociente incremental en los dos primeros instantes, $u_{j,1}=u_{j,0}+k g(x_j)$.\\
...
En esta sección nos centraremos en un concepto altamente relacionado con la estabilidad y energía del modelo \eqref{ecuaciononda}-\eqref{eq:frontera3}, tal y como se puede ver en \citep{mcowen}, el dominio de dependencia.
...
\begin{itemize}
	\item[a)] Que el dominio de dependencia quedase en el interior de los puntos de $[a,b]$ utilizados en el esquema en diferencias finitas (Coloreados de verde):
...
Haremos ahora el desarrollo de Taylor para cada una de las componentes anteriores, denotando para las derivadas a partir de orden dos  $u_{xxx}=u_{3x}$ y $u_{ttt}=u_{3t}$, respectivamente. Suponiendo que nuestra solución $u$ sea suficientemente regular, es decir, que se pueda derivar tantas veces como sea necesario, y denotando por brevedad $u=u(x_j,t-r)$, obtenemos.
\begin{equation*}
	\begin{split}
		...
		u_(x_j,t_{r-1})=&\ u-ku_{t}+k^2\frac{u_{tt}}{2}-k^3\frac{u_{3t}}{3!}+k^4\frac{u_{4t}}{4!}-k^5\frac{u_{5t}}{5!}+O(k^6),\\ 
		u(x_j,t_r)\ \ =&\ u.
	\end{split}
\end{equation*}
...
Volviendo a \eqref{eq=ondaesquemaconsistencia}, obtenemos:

\begin{equation*}
	\begin{split}
		T_{h,k}u:=&u_{tt}+2\frac{k^2 u_{4t}}{4!}+2\frac{k^4 u_{6t}}{6!}-c^2u_{xx}-2c^2 \frac{h^2 u_{4x}}{4!}-2c^2\frac{h^4 u_{6x}}{6!}\dots=\\
...
Como hemos observado que se da la convergencia del esquema para la solución exacta, la ecuación de propagación de los errores será:\\
...
\begin{itemize}
	...
	luego nuestro esquema es condicionalmente estable.
			
\end{itemize}
Veamos que condición se debe imponer en $\mu$ para que se tenga $$ -1\leq A\leq 1$$, y por tanto el esquema sea estable.
Definimos $s^2 \triangleq sin^2\left(\frac{\beta h}{2}\right)$, con esto tenemos
$$0\leq s^2\leq 1,$$
luego,
...
en lugar de la del producto interior anteriormente utilizado.\\
...
Sin embargo, en este caso nos beneficiaremos enormemente de utilizar $ D=\{x_0,x_1,\dots, x_{m-1}\}$. Para ello deberemos analizar como definir nuestro esquema para los puntos de la forma $(x_0,t_r)$, con $r=1,\dots, n-1$. Evaluemos la ecuación \ref{ecuaciononda}, en el punto $(x_0,t_r)$
  
$$u_{tt}(x_0,t_r)=c^2u_{xx}(x_0,t_r).$$
  
Utilizando la condición \eqref{eq:frontera3}, obtenemos,
  
$$0=c^2u_{xx}(x_0,t_r),$$
y por lo tanto para que se verifique nuestro esquema, la aproximación en dicho punto, tendrá que ser,
...
\begin{equation*}
	u_{-1,r} \triangleq -u_{1,r},
\end{equation*}
 y volviendo a utilizar \eqref{eq:esquemafrontera3}, hará que se verifique nuestro esquema en el punto $(x_0,t_r)$.\\
 
Retomando el tema que nos atañe, la energía en nuestro esquema, tomando $D=\{x_0,x_1,\dots, x_{m-1}\}$, y utilizando en este caso la función $\delta$ definida en \ref{defdelta}, para hacer el producto interior discreto con $\delta_tu$, tendremos.
...
Ahora para desarrollar el segundo termino de la igualdad usaremos la observación \ref{derivada2} y posteriormente el lema \ref{SumaPartes}.
...
$$\delta_{t}E(u)= \delta_t\left(||\delta_tu_{\cdot.r}||_D^2+||\delta_{x^-}u_{\cdot.r}||_D^2\right)=\mathfrak{b}.$$

Por lo tanto, para que se conserve la energía tendrá que ser $\mathfrak{b}=0$. Veamos que es así. Utilizando la condición de contorno para \eqref{eq:esquemafrontera3}, para $r=1,\dots, n-1$,

$$\delta_{t} u_{0,r}= \frac{1}{k}\left(u_{0,r+1}-u_{0,r-1}\right)=0,$$

$$\delta_{t} u_{m,r}= \frac{1}{k}\left(u_{m,r+1}-u_{m,r-1}\right)=0.$$

Luego nuestro esquema conserva la energía.
